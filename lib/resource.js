...
function Resource(db, config) {
  ...
  // application specific state for use in hooks
  this.app = config.app || {};
...
Resource.prototype.load = function(id, callback) {
  ...
  this.db.get(id, function(err, doc) {
    ...
    self._runHook('read', doc, function(err, doc) {
      if (err) return callback(err);
      callback(null, doc);
    });
  });
...
//
// Alias for the 'all' view
//

Resource.prototype.all = function(params, callback) {

  if (typeof params === 'function') {
    callback = params;
    params = {};
  }
  this.view('all', params, callback);
};
...
Resource.prototype.save = function(doc, callback) {
  ...
  var self = this;
  var hookName = doc._rev ? 'update' : 'create';
  
  this._runHook(hookName, doc, function(err, doc) {
    if (err) return callback(err);
...
Resource.prototype.destroy = function(doc, callback) {
  ...
  this._runHook('destroy', doc, function(err, doc) {
    if (err) return callback(err);
...
Resource.prototype._runHook = function(name, doc, callback) {

  if (this.hooks[name]) {
    // call hook with this scope
    return this.hooks[name].call(this, doc, callback);
  }
  // no hook
  callback(null, doc);
};
